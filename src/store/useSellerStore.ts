...
export const useSellerStore = create<SellerState>((set, get) => ({
  ...
  initIndexedDb: async () => {
    try {
      const dbHistory = await persistentStorage.getItem<QuoteHistoryItem[]>(QUOTE_HISTORY_KEY, []);
      if (dbHistory && dbHistory.length > 0) {
        set({ quoteHistory: dbHistory });
      }
    } catch {}
  },
  ...
  login: (vendedorNome, vendedorId) => {
    ...
    persistentStorage.setItem(SELLER_STORAGE_KEY, sessionData);
    set({ session: sessionData, isSellerLoggedIn: true });
  },

  logout: () => {
    persistentStorage.removeItem(SELLER_STORAGE_KEY);
  ...
  saveQuoteToHistory: (quote) => {
    ...
    const updated = [quote, ...history].slice(0, 100);
    persistentStorage.setItem(QUOTE_HISTORY_KEY, updated);
